...
var RTCPeerConnection = require ('react-native-webrtc').RTCPeerConnection
var RTCIceCandidate = require ('react-native-webrtc').RTCIceCandidate
var RTCSessionDescription = require ('react-native-webrtc').RTCSessionDescription

// import react-native-webrtc module
var getUM = require('react-native-webrtc')
...
/* Antistatic */
// пришлось закомментить нижеследующий кусок кода не работал
    /* pc.createOffer(function (desc) {
      pc.setLocalDescription(desc, function () {
        logger.info('offer::origPlanB->UnifiedPlan', dumpSDP(
          pc.localDescription))
        callback(null, pc.localDescription.sdp, self.processAnswer.bind(
          self))
  ...
  this.showLocalVideo = function () {
    /**
     * Not needed here, local video is managed in the upper level in the View
     */
  ...
  if (mode !== 'recvonly' && !videoStream && !audioStream) {
    function getMedia(constraints) {
     ...
     var onmystream = options.onmystream;
     getUM.getUserMedia(constraints).then(function (stream) {
        console.log("getUm callback");
        videoStream = stream;
        if (onmystream){
          onmystream(stream);
        }
...
WebRtcPeer.prototype.dispose = function () {
  ...
  try {
    ...
    if (pc) {
      ...
      if (typeof pc.getLocalStreams === 'function')
